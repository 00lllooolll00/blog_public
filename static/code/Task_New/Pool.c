...
/** @brief 空闲块链表头和尾 */
static mem_block_t free_list_start, *free_list_end = NULL;

/** @brief 内存池统计信息 */
static pool_stats_t pool_statistics = {0};
...
static void _insert_free_block(mem_block_t *block_to_insert);
static mem_block_t *_find_suitable_block(size_t wanted_size);
static void _split_block(mem_block_t *block, size_t wanted_size);
...
static void _init_heap(void)
{
    ...
    /* 创建起始标记块 */
    free_list_start.next_free = (mem_block_t *)aligned_heap;
    free_list_start.block_size = 0;

    /* 设置结束标记块的位置 */
    free_list_end = (mem_block_t *)(aligned_heap + total_heap_size - sizeof(mem_block_t));
    free_list_end->next_free = NULL;
    free_list_end->block_size = 0;
    free_list_end->block_size = SET_ALLOCATED(free_list_end->block_size);

    /* 创建第一个大的空闲块 */
    first_block = (mem_block_t *)aligned_heap;
    first_block->next_free = free_list_end;
    first_block->block_size = total_heap_size - sizeof(mem_block_t);
...
static void _insert_free_block(mem_block_t *block_to_insert)
{
    mem_block_t *current = &free_list_start;
...
static mem_block_t *_find_suitable_block(size_t wanted_size)
{
    mem_block_t *current, *prev = &free_list_start;
...
static void _split_block(mem_block_t *block, size_t wanted_size)
{
    ...
    {
        /* 创建新的空闲块 */
        new_block = (mem_block_t *)((uint8_t *)block + wanted_size);
...
void *MemPool_Malloc(size_t size)
{
    ...
    /* 计算实际需要的大小(包含头部信息并对齐) */
    wanted_size = ALIGN_UP(size + sizeof(mem_block_t));
    ...
    if (block != NULL)
    {
        ...
        /* 返回用户可用的内存地址(跳过头部) */
        return_ptr = (void *)((uint8_t *)block + sizeof(mem_block_t));
    }
